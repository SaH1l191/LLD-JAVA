...
👉 You are making a promise that:

The class is Car

You know its constructor

You know its dependencies

You are okay changing this line whenever Car changes

Now let’s SEE this pain in real code.

❌ WITHOUT FACTORY — REALISTIC EXAMPLE
1️⃣ Suppose your project structure
Main.java
...
2️⃣ Object creation scattered everywhere (Problem 1)
Main.java
Vehicle v = new Car();
...
⚠️ Car creation logic is copied everywhere

This is what we mean by:

❌ Object creation is scattered everywhere

3️⃣ High Coupling — WHAT DOES IT MEAN?

Coupling = how strongly your code depends on another code

This line:
Vehicle v = new Car();


Means the class depends on Car, not Vehicle.

So each file is tightly coupled to:
...
4️⃣ Now let’s add a REAL change (this is where pain begins)
🔄 New Requirement
...
5️⃣ WHAT BREAKS WITHOUT FACTORY 💥
Main.java ❌
Vehicle v = new Car(); // ❌ compile error
...
6️⃣ Client responsibility (THIS is the key misunderstanding)
Without Factory, client must know:
Responsibility	Why it’s bad
Which class?	Client chooses Car, Truck, Bus
How to create?	Client calls constructor
Which constructor?	Changes break code
What dependencies?	Engine, Fuel, Config

👉 Client is doing too much

✅ NOW SAME EXAMPLE WITH FACTORY
7️⃣ Centralized creation
VehicleFactory.java
public class VehicleFactory {
    public static Vehicle createVehicle(String type) {
        if (type.equals("Car")) {
            return new Car(new Engine(), FuelType.PETROL);
        }
        if (type.equals("Bus")) {
            return new Bus(new Engine(), FuelType.DIESEL);
        }
        throw new IllegalArgumentException();
    }
}

8️⃣ Client code (everywhere)
Main.java
...
9️⃣ Now change constructor again 🔄
public Car(Engine engine, FuelType fuelType, GPS gps) {
}
...
This is why Factory matters.

🧠 Translate to interview language
Problem 1: Object creation is scattered

Multiple classes directly instantiate new Car(), duplicating creation logic and making changes risky.

Problem 2: High coupling

Client code depends on concrete classes and their constructors instead of abstractions.

Factory Solution

Factory centralizes object creation, reducing coupling and limiting change impact to a single location.

🧪 Simple mental rule (remember this)

If new appears in many places → you need a Factory
