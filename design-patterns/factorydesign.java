...
Why NOT directly do new Car() everywhere?
❌ Without Factory
Car car = new Car();
car.drive();


Looks fine… until scale hits.

Now imagine:

Car constructor changes

Car needs extra dependencies

Car is replaced by ElectricCar

You want MockCar in tests

You now must change every place where new Car() exists.

💥 That is tight coupling

2️⃣ What does “Client depends on Interface” REALLY mean?
❌ Tight coupling (bad)
Car car = new Car();
...
📌 This is what “client depends on interface” means
Not theory — blast-radius control.

3️⃣ “But factory ALSO uses new Car() 🤔”

Yes. And that’s the point.

Factory is the ONLY place where new is allowed.
...
Think of factory as quarantine for object creation ☣️

4️⃣ Open–Closed Principle (with and without Factory)
❌ Without Factory
if (type.equals("CAR")) {
    return new Car();
    ...
    return new Truck(); // added later
}


You must modify existing code → ❌ OCP violation
...
“Builder, give me a house”

Builder internally decides:
...
If materials change → you don’t care

🧠 Factory = Builder contract